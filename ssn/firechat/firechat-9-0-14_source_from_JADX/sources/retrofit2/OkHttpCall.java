...
final class OkHttpCall<T> implements Call<T> {
    ...
    public void enqueue(final Callback<T> callback) {
        Call call;
        Throwable th;
        C3226Utils.checkNotNull(callback, "callback == null");
        synchronized (this) {
            if (this.executed) {
    ...
    public C3224Response<T> execute() throws IOException {
        Call call;
        synchronized (this) {
            if (this.executed) {
                throw new IllegalStateException("Already executed.");
    ...
    /* access modifiers changed from: 0000 */
    public C3224Response<T> parseResponse(C3012Response response) throws IOException {
        ResponseBody body = response.body();
        C3012Response build = response.newBuilder().body(new NoContentResponseBody(body.contentType(), body.contentLength())).build();
        int code = build.code();
        if (code < 200 || code >= 300) {
            try {
                return C3224Response.error(C3226Utils.buffer(body), build);
            } finally {
                body.close();
            }
        } else if (code == 204 || code == 205) {
            body.close();
            return C3224Response.success(null, build);
        } else {
            ExceptionCatchingRequestBody exceptionCatchingRequestBody = new ExceptionCatchingRequestBody(body);
            try {
                return C3224Response.success(this.serviceMethod.toResponse(exceptionCatchingRequestBody), build);
