...
class Camera1Session implements CameraSession {
    ...
    public static void create(CreateSessionCallback createSessionCallback, Events events2, boolean z, Context context, SurfaceTextureHelper surfaceTextureHelper2, int i, int i2, int i3, int i4) {
        ...
        try {
            ...
            if (open == null) {
                createSessionCallback2.onFailure(FailureType.ERROR, "Camera not found");
                return;
            }
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        Camera1Session camera1Session = new Camera1Session(events2, z2, context, surfaceTextureHelper2, i5, open, cameraInfo, findClosestCaptureFormat, nanoTime);
                    ...
                    open.release();
                    createSessionCallback2.onFailure(FailureType.ERROR, runtimeException.getMessage());
                }
            } catch (IOException e3) {
                IOException iOException = e3;
                open.release();
                createSessionCallback2.onFailure(FailureType.ERROR, iOException.getMessage());
            }
        } catch (RuntimeException e4) {
            createSessionCallback2.onFailure(FailureType.ERROR, e4.getMessage());
        }
    }
    ...
    private Camera1Session(Events events2, boolean z, Context context, SurfaceTextureHelper surfaceTextureHelper2, int i, Camera camera2, CameraInfo cameraInfo, CaptureFormat captureFormat2, long j) {
        String str = TAG;
        StringBuilder sb = new StringBuilder();
    ...
    private void startCapturing() {
        Logging.m314d(TAG, "Start capturing");
        checkIsOnCameraThread();
        ...
        this.camera.setErrorCallback(new ErrorCallback() {
            public void onError(int i, Camera camera) {
                ...
                Logging.m315e(Camera1Session.TAG, str);
    ...
    public void stopInternal() {
        ...
        checkIsOnCameraThread();
        if (this.state == SessionState.STOPPED) {
            Logging.m314d(TAG, "Camera is already stopped");
            return;
        }
        ...
        this.surfaceTextureHelper.startListening(new OnTextureFrameAvailableListener() {
            public void onTextureFrameAvailable(int i, float[] fArr, long j) {
                ...
                if (Camera1Session.this.state != SessionState.RUNNING) {
                    Logging.m314d(Camera1Session.TAG, "Texture frame captured but camera is no longer running.");
                ...
                int access$800 = Camera1Session.this.getFrameOrientation();
                if (Camera1Session.this.info.facing == 1) {
                    fArr = RendererCommon.multiplyMatrices(fArr, RendererCommon.horizontalFlipMatrix());
                }
                Camera1Session.this.events.onTextureFrameCaptured(Camera1Session.this, Camera1Session.this.captureFormat.width, Camera1Session.this.captureFormat.height, i, fArr, access$800, j);
            }
        });
    }
        ...
        this.camera.setPreviewCallbackWithBuffer(new PreviewCallback() {
            public void onPreviewFrame(byte[] bArr, Camera camera) {
                ...
                if (camera != Camera1Session.this.camera) {
                    Logging.m315e(Camera1Session.TAG, "Callback from a different camera. This should never happen.");
                } else if (Camera1Session.this.state != SessionState.RUNNING) {
                    Logging.m314d(Camera1Session.TAG, "Bytebuffer frame captured but camera is no longer running.");
                } else {
                    ...
                    Camera1Session.this.events.onByteBufferFrameCaptured(Camera1Session.this, bArr, Camera1Session.this.captureFormat.width, Camera1Session.this.captureFormat.height, Camera1Session.this.getFrameOrientation(), nanos);
